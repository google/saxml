...
func join(saxCell, ipPort, debugAddr, dataAddr string, specs *pb.ModelServer, adminPort int) error {
	...
	return location.Join(context.Background(), saxCell, ipPort, debugAddr, dataAddr, specs, adminPort)
}

//export sax_join
func sax_join(saxCellPtr *C.char, saxCellSize C.int, ipPortPtr *C.char, ipPortSize C.int, debugAddrPtr *C.char, debugAddrSize C.int, dataAddrPtr *C.char, dataAddrSize C.int, specsPtr unsafe.Pointer, specsSize C.int, adminPort int) *C.char {
	...
	dataAddr := C.GoStringN(dataAddrPtr, dataAddrSize)
	...
	if err := join(saxCell, ipPort, debugAddr, dataAddr, specs, adminPort); err != nil {
