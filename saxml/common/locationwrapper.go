...
func join(saxCell string, ipPort string, specs *pb.ModelServer, debugPort, adminPort int) error {
	...
	return location.Join(context.Background(), saxCell, ipPort, specs, debugPort, adminPort)
}

//export sax_join
func sax_join(saxCellPtr *C.char, saxCellSize C.int, ipPortPtr *C.char, ipPortSize C.int, specsPtr unsafe.Pointer, specsSize C.int, debugPort, adminPort int) *C.char {
	...
	if err := join(saxCell, ipPort, specs, debugPort, adminPort); err != nil {
