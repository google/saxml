...
// Package cloudenv defines a Cloud environment.
package cloudenv
...
const (
	...
	// SaxRoot is the name of an environment variable whose value is used as Sax root if the
	// --sax_root flag is not set. This is for when the location wrapper is embedded in model servers.
	SaxRoot = "SAX_ROOT"
)
...
func (e *Env) Init(ctx context.Context) {
	...
	if err != nil {
		log.Error("Error retrieving Google Cloud credentials, can only access local files from now on: ", err)
		return
	}
	...
	if err != nil {
		log.Error("Error creating a Google Cloud Storage client, further access will fail: ", err)
		return
	}
}
