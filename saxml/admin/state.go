...
func (s *State) Load(ctx context.Context, fullName naming.ModelFullName, spec *apb.Model, waiter *waitable.Waitable) error {
	...
	log.V(2).Printf("Loading model %v with %v", fullName, spec)
	model := newModel(spec)
	s.wanted[fullName] = model
	log.Printf("Enqueuing action %v of model %v on state %p, queue size %d", load, fullName, s, s.queue.Size())
...
func (s *State) Unload(ctx context.Context, fullName naming.ModelFullName, waiter *waitable.Waitable) error {
	...
	for existing, model := range s.wanted {
		if existing == fullName {
			log.V(2).Printf("Unloading model %v", fullName)
			delete(s.wanted, fullName)
			log.Printf("Enqueuing action %v of model %v on state %p, queue size %d", unload, fullName, s, s.queue.Size())
...
func (s *State) act(a *action) {
	...
	case wakeup:
		...
		if _, err := s.client.WakeUp(ctx, &mpb.WakeUpRequest{}); err != nil {
			log.V(1).Printf("Failed to wake up server %v (%v)", s.Addr, err)
		}
	default:
...
func (s *State) initialize(ctx context.Context, modelFinder ModelFinder) error {
	...
	log.V(3).Printf("The server sees %v", seen)
