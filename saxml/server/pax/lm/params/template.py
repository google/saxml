...
# TODO(sax,zqfeng): consolidate with set_decoding_sharding_hparams()
def set_decoding_sharding_hparams_for_collective_matmul(
    task_p, mesh_shape, decode_mesh_transpose
):
  """Set spmd sharding hparams to make it amenable to collective matmul."""
  model_p = task_p.model

  replica_axis = 'replica'
  data_axis = 'data'
  mdl_axis = 'mdl'
  fprop_mdl_axis = 'fprop_mdl'
  batch_axes = (replica_axis, data_axis)

  mesh_axis_names = [replica_axis, data_axis, fprop_mdl_axis]
  if decode_mesh_transpose:
    mesh_axis_names = [replica_axis, data_axis, mdl_axis, fprop_mdl_axis]
    mesh_shape = [mesh_shape[0], mesh_shape[1], 1, mesh_shape[2]]
  model_p.mesh_axis_names = mesh_axis_names
  ...
  w_df = [data_axis, maybe_mdl(fprop_mdl_axis)]
  w_dnh = [data_axis, maybe_mdl(fprop_mdl_axis), None]
  w_vd = [maybe_mdl(fprop_mdl_axis), data_axis]
  a_bld = [batch_axes, None, fprop_mdl_axis]
  a_blf = [batch_axes, None, maybe_mdl(fprop_mdl_axis)]
  a_blnh = [batch_axes, None, maybe_mdl(fprop_mdl_axis), None]
  a_blv = [batch_axes, None, maybe_mdl(fprop_mdl_axis)]

  if model_p.lm_tpl.cls != transformer_models.TransformerLm:
    raise NotImplementedError(f'Unsupported {str(model_p.lm_tpl.cls)}')

  lm_cls = cast(
      Type[transformer_models.TransformerLm],
      pax_fiddle.get_callable(model_p.lm_tpl),
  )
  emb_lookup_style = model_p.lm_tpl.softmax_tpl.lookup_style
  model_p.lm_tpl = lm_cls.set_custom_sharding_params(
      model_p.lm_tpl,
      ici_mesh_shape=mesh_shape,
      dcn_mesh_shape=None,
  ...
  # Restore the embedding lookup style as set_custom_sharding_params may change
  # its value.
  model_p.lm_tpl.softmax_tpl.lookup_style = emb_lookup_style
  model_p.ici_mesh_shape = mesh_shape
  model_p.dcn_mesh_shape = None
